"""Configuration management for WCAG Backend V2."""
...
class ScanningConfig(BaseSettings):
    ...
    default_scanners: List[str] = ["axe", "html_validator", "contrast", "keyboard", "aria"]
...
class SecurityConfig(BaseSettings):
    ...
    jwt_access_expiry: int = 900       # 15 minutes in seconds
    jwt_refresh_expiry: int = 604800   # 7 days in seconds
...
def load_config(config_path: Optional[Union[str, Path]] = None) -> Config:
    ...
    if config_path is None:
        # Default to wcag-backend/config.yaml
